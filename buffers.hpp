...
//!
inline void cudaMemcpyCheck(cudaError_t status){
    if (status != 0)
    {
        LOG(ERROR) << "Cuda failure: " << status;
        abort();
    }
}
...
class BufferManager {
   public:
    ...
    //!
    BufferManager(nvinfer1::ICudaEngine* engine, const int batchSize = 0,
    ...
    void memcpyBuffers(const bool copyInput, const bool deviceToHost, const bool async, const cudaStream_t& stream = 0) {
        for (int i = 0; i < mEngine->getNbBindings(); i++) {
            ...
            if ((copyInput && mEngine->bindingIsInput(i)) || (!copyInput && !mEngine->bindingIsInput(i))) {
                if (async)
                    cudaMemcpyCheck(cudaMemcpyAsync(dstPtr, srcPtr, byteSize, memcpyType, stream));
                else{
                    cudaMemcpyCheck(cudaMemcpy(dstPtr, srcPtr, byteSize, memcpyType));
                }
            }
        }
    }
